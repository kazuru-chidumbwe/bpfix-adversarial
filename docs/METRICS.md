...
| VerifierState (VS) | **Captured verifier log** only | Parses `; text @ path:line` maps already present in the log. VS itself does not read the object; the kernel derives those maps from the object's BTF line info |
