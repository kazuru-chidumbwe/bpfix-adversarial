...
- **Once a tag is cited in a manuscript, it must not be force-moved.** Ship fixes as a new SemVer tag instead of rewriting history. _Exception (2026-09-14): `v1.0.2` was re-cut once to fold in offline-only refinements so the cited tree matches the already-submitted minor-revision response; headline results unchanged (see CHANGELOG). The interim `v1.0.3` label was retired._ _Second exception (2026-09-19): `v1.0.2` was re-cut from `899f112` because the cited tree contradicted the minor-revision response letter and the revised manuscript, both of which name `v1.0.2` and cannot be re-issued (for example, the version DOI was missing from the tagged `CITATION.cff` and `codemeta.json`, and several committed results still used retracted wording). Corrections are listed in the CHANGELOG; the paper's template tallies are unchanged. Existing clones: `git fetch --tags --force`. This is the last move: any later fix ships as a new tag._
